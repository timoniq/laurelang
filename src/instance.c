...
gen_resp array_generate(
    ...
    debug("array_generate called, state = %s\n", im->state == I ? "I" : "U");
    ...
    if (im->state == I) {
        ...
        debug("Generating uninstantiated array, setting up domain generation\n");
        array_det ctx[1];
        ctx->final_rec = rec;
        ctx->final_external_ctx = external_ctx;
        ctx->im = im;
        ...
        debug("Starting int_domain_generate for array length\n");
        gen_resp response = int_domain_generate(len_copy, array_length_receiver, ctx);
        debug("int_domain_generate returned, response.r = %d\n", response.r);
